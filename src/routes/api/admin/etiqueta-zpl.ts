// Proxy do ZPL da etiqueta, servido pelo NOSSO domínio.
//
// POR QUE UMA ROTA DE API E NAO UMA SERVER FN:
// server fn devolve JSON serializado; aqui o corpo precisa ser
// `application/octet-stream` com os bytes crus, para o navegador baixar/imprimir direto.
//
// POR QUE PROXY E NAO REDIRECT:
// GET /me/imprimir/zpl/{id} devolve uma url pre-assinada do S3 que expira em
...
// 10 ids com 3 falhas NAO devolve um ZPL de 7 etiquetas em silencio. O corpo vem
...
export const Route = createFileRoute("/api/admin/etiqueta-zpl")({
  server: {
    handlers: {
      GET: async ({ request }) => {
        try {
          ...
          const sources: LabelZplSource[] = ids.map((id) => {
          ...
          const { buscarZplEtiqueta } = await import("@/lib/melhor-envio-client.server");

          // Em paralelo: o gargalo e o download do S3 (~800ms cada), que
          // paraleliza bem. Uma etiqueta mediu ~1,5s no total.
          const results: LabelZplFetchResult[] = await Promise.all(
            sources.map(async (source): Promise<LabelZplFetchResult> => {
              ...
              const fetched = await buscarZplEtiqueta(source.externalId);
              return fetched.ok
                ? { ok: true, source, zpl: fetched.zpl }
          ...
          // Marca impressao apenas do que REALMENTE entrou no ZPL.
          ...
          const headers: Record<string, string> = {
            ...
            "content-disposition": `attachment; filename="etiquetas-${printedIds.length}.txt"`,
            "cache-control": "no-store",
            "X-Etiquetas-Incluidas": String(merged.included.length),
            "X-Etiquetas-Total": String(sources.length),
            "X-Etiquetas-Count": String(merged.count),
          };
          ...
          return new Response(merged.zpl, { status: 200, headers });
        } catch (e: unknown) {
