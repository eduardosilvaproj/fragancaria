...
| C14 | **Tela Novo Produto não tem campos de custo e margem** — a tela de edição recebeu `cost`, `target_margin` e modo `manual`/`auto`; a criação ainda obriga a cadastrar e editar em seguida para preencher custo. | pendente | `src/routes/admin/produtos/novo.tsx` |
| C16 | **Lista de categorias e marcas do menu fixa no código (`NavbarEditorial.tsx`)** — causa raiz que exigiu remoção manual de Truss, Lowell e Kérastase que ficaram com zero produtos ativos. A correção real é gerar a lista dinamicamente a partir do catálogo com um mínimo de produtos ativos (ficará para a Fase 4 junto com a esteira de marcas). | pendente | `NavbarEditorial.tsx` |
