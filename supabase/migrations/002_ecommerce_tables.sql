-- ============================================
-- HISTÓRICO: aplicado parcialmente/evoluído fora deste arquivo.
-- NÃO reflete produção. Fonte da verdade: ver
-- supabase/schema-baseline-*.sql. (R4)
--
-- Resultado da auditoria 2026-07-08:
--   Tabelas que EXISTEM em prod:
--     customers (presente; pode ter colunas extras), orders (presente
--       com 28 colunas — este arquivo declara 49, divergência grande:
--       producao usa amount/auth_user_id/customer_cpf/discount/metadata/
--       payer_email/raw/refund_status/shipping_price/status_history
--       e NAO tem affiliate_id/affiliate_commission/billing_address/
--       cancel_reason/cancelled_at/coupon_code/coupon_id/customer_document/
--       customer_id/customer_notes/delivered_at/discount_amount/
--       fulfillment_status/internal_notes/order_number/paid_at/
--       payment_details/shipped_at/shipping_carrier/shipping_cost/
--       shopify_order_id/tracking_url).
--   Tabelas que NAO EXISTEM em prod (nao aplicar este arquivo
--   do zero; seriam criadas sem motivo):
--     products, coupons, order_items, stock_movements.
--   Tabelas presentes em prod que NAO estao neste arquivo:
--     admins, conversations, messages, notifications,
--     refund_requests, wishlist, home_featured_manual.
--
--   Recomendacao: nao executar este arquivo do zero. Para ambientes
--   novos, usar o schema-baseline.
-- ============================================
