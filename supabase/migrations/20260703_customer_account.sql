-- 20260703_customer_account.sql
-- Adiciona area do cliente: link orders <-> auth.users, wishlist, notificacoes,
-- solicitacoes de estorno.
...
-- 3. Tabela notifications (avisos do sistema para o cliente)
CREATE TABLE IF NOT EXISTS public.notifications (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  user_id UUID NOT NULL REFERENCES auth.users(id) ON DELETE CASCADE,
  type TEXT NOT NULL,                -- 'order_shipped' | 'order_delivered' | 'points_earned' | 'refund_status' | 'order_created'
  title TEXT NOT NULL,
...
-- 4. Tabela refund_requests (cliente pede, admin aprova via service_role)
...
-- UPDATE: cliente so pode cancelar o proprio pedido quando ainda esta pending.
...
-- 5. orders.refund_status: status visual para o cliente
ALTER TABLE public.orders
...
-- 6. RLS para orders: usuario logado ve pedidos com auth_user_id dele OU com seu email.
